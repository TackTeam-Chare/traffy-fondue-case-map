...
const Footer = ({ onOpenHistory, onOpenSearch }) => {
  return (
    <footer className="fixed bottom-0 left-0 right-0 bg-emerald-800/90 backdrop-blur-md text-white flex justify-around items-center py-3 shadow-lg z-50">
      {/* หน้าแรก */}
      <a href="/" className="flex flex-col items-center text-sm hover:text-yellow-400 transition">
        <MdHome size={24} />
        <span>หน้าแรก</span>
      </a>
...
<button
      ...
      {/* แผนที่ */}
      <a href="/map" className="flex flex-col items-center text-sm hover:text-yellow-400 transition">
        <FaMapMarkedAlt size={24} />
        <span>แผนที่</span>
      </a>

      {/* ภารกิจ */}
      <a href="/missions" className="flex flex-col items-center text-sm hover:text-yellow-400 transition">
